...
public class ObjectPool {
    ...
    private static ObjectPool _inst;
    public static ObjectPool intance { get {
            if(_inst == null) {
                _inst = new ObjectPool();
            }
            return _inst;
        }
    }
    ...
    public static void Destroy() {
        if(_inst != null) {
            foreach(KeyValuePair<string, List<GameObject>> pool in _inst.objectPools) {
                pool.Value.Clear();
            }

            _inst.objectPools.Clear();
        }
    }
     ...
     */
    private const int DEFAULT_POOL_SIZE = 1;
    ...
    public static GameObject GetActiveInitialized(GameObject prototype, Vector3 position, Quaternion rotation) {
        GameObject obj = intance.GetInactiveGameObjectInstance(prototype);
    ...
    public static T GetActive<T>(T prototype, Vector3 position, Quaternion rotation) where T : MonoBehaviour {
        GameObject obj = GetActiveInitialized(prototype.gameObject, position, rotation);
